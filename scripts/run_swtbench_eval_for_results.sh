...
EVAL_BUILD_MODE="${EVAL_BUILD_MODE:-cli}"
...
python - "$REPORT_JSON" "$LOCAL_REPORT_TXT" <<'PY'
import json
import sys
...
total = int(data.get("total_instances", 0) or 0)
...
lines = [
    ...
    "- 分母是本次生成并提交给 SWT-Bench 的 predictions 数量，不是原始数据集总数。",
]
out_path.write_text("\n".join(lines) + "\n")
print("\n".join(lines))
PY
