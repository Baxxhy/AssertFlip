...
from .btcr import build_btcr_context
from .btcr.io import dump_json
...
def select_dataset_instance(dataset_obj, instance_id: str | None = None, dataset_index: int = 0):
    ...
    if instance_id:
        for item in dataset_obj:
            if isinstance(item, dict) and item.get("instance_id") == instance_id:
                return item
        raise ValueError(f"Unable to find instance_id={instance_id} in dataset")
    if dataset_index < 0 or dataset_index >= len(dataset_obj):
        raise ValueError(f"--dataset-index {dataset_index} is out of range for dataset of size {len(dataset_obj)}")
    return dataset_obj[dataset_index]
...
def parse_args(args=None):
    ...
    ap.add_argument('--dataset-index', type=int, default=0,
                    help='index to use when --dataset is a list')
    ap.add_argument('--instance-id', type=str, default=None,
                    help='instance_id to use when --dataset is a list')
    ...
    ap.add_argument('--btcr', default=True,
                    action=argparse.BooleanOptionalAction,
                    help='enable Backward Trigger Condition Recovery context')
    ap.add_argument('--related-tests', type=Path, default=Path('datasets/related_tests.json'),
                    help='path to iCoRe related tests JSON')
    ap.add_argument('--btcr-output-dir', type=Path, default=Path('btcr_results'),
                    help='directory for BTCR intermediate artifacts')
...
async def save_test(args, seg, test_code, mode):
    ...
    await save_btcr_final(args, seg, test_code, mode)
    state.inc_counter('G')

async def save_btcr_final(args, seg, test_code, mode):
    context = getattr(seg, "btcr_context", {}) or {}
    if not context:
        return
    artifact_dir = Path(context.get("artifact_dir") or (args.btcr_output_dir / seg.instance_id))
    artifact_dir.mkdir(parents=True, exist_ok=True)
    (artifact_dir / "final_test.py").write_text(test_code)
    dump_json(artifact_dir / "final_f_to_p_result.json", {
        "instance_id": seg.instance_id,
        "mode": mode,
        "status": "saved_final_test",
        "failure_type": None,
    })
...
def main():
    ...
    if args.prompt_for_tests:
        ...
        dataset = select_dataset_instance(dataset_obj, args.instance_id, args.dataset_index)
        selected_files = []
        objects = dataset.get("line_level_localization", [])
        for obj in objects:
        ...
        localized_code = dataset.get("localized_code", "")
        line_level_localization = dataset.get("line_level_localization", [])
        instance_id = dataset.get("instance_id", "unknown_instance")
        continue_from = dataset.get("continue_from", None)
        btcr_context = build_btcr_context(
            dataset=dataset,
            repo_root=Path.cwd(),
            source_dir=args.source_dir,
            related_tests_path=args.related_tests,
            output_dir=args.btcr_output_dir,
            enabled=args.btcr,
        )
