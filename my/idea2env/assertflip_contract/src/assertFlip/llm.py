...
# Register Ollama model to support function calling
...
class Chatter:
    ...
    def add_function(self, function: T.Callable) -> None:
        ...
        if "ollama" not in self._model.lower() and not litellm.supports_function_calling(self._model):
    ...
    def _request(self, messages: T.List[dict]) -> dict:
        return {
            ...
            **({'temperature': self._model_temperature} if self._model_temperature is not None else {}),
            'messages': messages,
            'max_tokens': 2048,
            **({'api_base': "http://swag-gpu:11434"} if "ollama" in self._model else {}),
        ...
        while attempts < max_attempts:
            try:
                if self.token_rate_limit:
                    try:
                        _prompt = count_tokens(self._model, request)
                        _budget = request.get("max_tokens", 0)
                        await self.token_rate_limit.acquire(_prompt + _budget)
                        needed = _prompt + _budget
                        await self.token_rate_limit.acquire(needed)
                    except ValueError as e:
                        self._log_msg(ctx, f"Error: too many tokens for rate limit ({e})")
                        print(f"Error: too many tokens for rate limit ({e})")
                        return None 
...
""")
    ...
    def _log_llm_chat(self, instance_id: str, messages: list, response: dict, ctx: object):
        ...
        path = results_dir / f"llm_calls_{instance_id}.jsonl"
    ...
    async def chat(self, messages: list, *, ctx: T.Optional[object] = None, response_format=None) -> dict|None:
        ...
        while func_calls <= self._max_func_calls_per_chat:
            ...
            if response_format:
                request['response_format'] = litellm.utils.type_to_response_format_param(
                    response_format
                )
            ...
            if response_format:
                if ctx is not None:
                    _response = json.loads(response.choices[0].message.content)
                    self._log_llm_chat(ctx.instance_id, messages, _response, ctx)
            ...
            if response.choices[0].finish_reason != "tool_calls":
                ...
                if content.startswith("<think>"):
                    if "</think>" in content:               
                        ...
                        _response["choices"][0]["message"]["content"] = content.strip()
                if ctx is not None:
                    self._log_llm_chat(ctx.instance_id, messages, _response, ctx)
                return _response
