...
try:
    from ecg import adapter as contract_adapter
except Exception:
...
def parse_args(args=None):
    ...
    ap.add_argument('--contract-brt', default=os.getenv("CONTRACT_BRT_ENABLE", "1") not in {"0", "false", "False", "no"},
                    action=argparse.BooleanOptionalAction,
                    help='enable Contract-BRT Environment Contract Graph preflight')
    ap.add_argument('--contract-root', type=Path, default=Path(os.getenv("CONTRACT_BRT_ROOT", ".")),
                    help='Contract-BRT root directory')
    ap.add_argument('--contract-dir', type=Path, default=Path(os.getenv("CONTRACT_BRT_CONTRACT_DIR", "contracts")),
                    help='directory for ECG artifacts')
    ap.add_argument('--generated-test-dir', type=Path, default=Path(os.getenv("CONTRACT_BRT_GENERATED_TEST_DIR", "generated_tests")),
                    help='directory for Contract-BRT generated test artifacts')
    ap.add_argument('--related-tests', type=Path, default=Path("datasets/related_tests.json"),
                    help='related tests JSON used by Contract-BRT ECG extraction')
    ap.add_argument('--contract-strictness', choices=['repair', 'warn', 'fail'], default=os.getenv("CONTRACT_BRT_STRICTNESS", "warn"),
                    help='how to handle Contract-BRT preflight failures')
    ap.add_argument('--contract-scaffold-mode', choices=['off', 'fallback', 'force'], default=os.getenv("CONTRACT_BRT_SCAFFOLD_MODE", "off"),
                    help='how Contract-BRT applies locked scaffold postprocessing')
...
async def ask_llm_to_invert_test(chatter, seg, prompter, test_code, attempt_log, attempt_num, retry_message: str = None):
    ...
    if response_message.get("content") and '```python' in response_message["content"]:
        ...
        if contract_adapter is not None:
            code = contract_adapter.postprocess_test_code(
                code, getattr(seg, "contract_context", None), label=f"invert_attempt_{attempt_num}"
            )
...
async def new_prompt(args, seg, chatter, prompter, *, mode: str, retry: bool = False):
    ...
    if contract_adapter is not None and args.contract_brt:
        messages = contract_adapter.augment_messages(messages, getattr(seg, "contract_context", None))
...
async def generate_test(args, chatter, prompter, seg, attempt_log, attempt_num, mode, messages, max_attempts=3):
    for sub_attempt in range(1, max_attempts + 1):
        ...
        if '```python' in content:
            ...
            if test_code:
                if contract_adapter is not None and args.contract_brt:
                    test_code = contract_adapter.postprocess_test_code(
                        test_code,
                        getattr(seg, "contract_context", None),
                        label=f"{mode}_attempt_{attempt_num}_{sub_attempt}",
                    )
                attempt_log.append({
                    "phase": "first_test",
                    "attempt": attempt_num,
...
async def run_test_and_capture(args, seg, test_code, attempt_log, phase, attempt):
    ...
    try:
        if contract_adapter is not None and args.contract_brt:
            report = contract_adapter.preflight_test_code(
                test_code,
                args=args,
                context=getattr(seg, "contract_context", None),
            ...
            if report is not None:
                ...
                if not report.passed and args.contract_strictness in {"repair", "fail"}:
                    feedback = contract_adapter.make_refinement_feedback(report)
                    test_attempt.update({"status": "error", "error": feedback, "test_code": test_code})
...
async def fix_test_loop(args, chatter, prompter, seg, test, attempt_log, phase, goal_status, validate_with_llm=False, error_prompt_mode="error_fix", messages=None, max_attempts= 10):
    ...
    while attempt <= args.max_attempts:
        ...
        if (
            contract_adapter is not None
            and args.contract_brt
            and prompt_error
            and goal_status == "passing"
            and phase == "generate_passing_test"
        ):
            env_feedback = contract_adapter.make_execution_environment_feedback(
                prompt_error,
                getattr(seg, "contract_context", None),
                test,
            )
            if env_feedback:
                attempt_log.append({
                    "phase": "contract_reactive_env_repair",
                    "parent_phase": phase,
                    "attempt": attempt,
                    "triggered": True,
                    "error_kind": contract_adapter.classify_failure(prompt_error),
                    "status_before_repair": status,
                })
                prompt_error += env_feedback
        ...
        if "```python" in response_message["content"]:
            ...
            if contract_adapter is not None and args.contract_brt:
                test = contract_adapter.postprocess_test_code(
                    test,
                    getattr(seg, "contract_context", None),
                    label=f"{phase}_repair_{attempt}",
                )
...
async def save_attempt_log(seg, attempt_log):
    results_dir = Path(os.getenv("ASSERTFLIP_RESULTS_DIR", "/results"))
    attempt_path = results_dir / f"attempts_{seg.instance_id}.json"
...
def main():
    ...
    global state
    args = parse_args()
    os.environ["CONTRACT_BRT_SCAFFOLD_MODE"] = args.contract_scaffold_mode
    ...
    if args.prompt_for_tests:
        ...
        if contract_adapter is not None and args.contract_brt:
            contract_context = contract_adapter.prepare_instance_context(
                args=args,
                dataset=dataset,
                repo_root=Path.cwd(),
                test_dir=str(args.tests_dir),
                runner=args.test_cmd,
            )
        ...
        segments[0].contract_context = contract_context
